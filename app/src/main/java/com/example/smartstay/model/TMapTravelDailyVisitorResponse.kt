...
data class TMapTravelDailyVisitorResponse(
    val status: CommonStatus,
    val contents: DailyVisitorContents
)

data class DailyVisitorContents(
    ...
    val raw: DailyRaw // 지정한 검색 조건에 따른 추정 여행자 수 정보
)

data class DailyRaw(
    val travelerCount: Int, // 추정 여행자 수
    val date: String, // 추정 여행자 수 정보가 검색된 일자(YYYYMMDD)
)
